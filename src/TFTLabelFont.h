const uint8_t TFTLabelFontBitmaps[] PROGMEM = {
  
};

const GFXglyph TFTLabelFontGlyphs[] PROGMEM = {
  {     0,   0,   0,   5,    0,    1 }   // ' '
 ,{     0,   2,  13,   6,    2,  -12 }   // '!'
 ,{     4,   5,   4,   6,    1,  -12 }   // '"'
 ,{     7,  10,  12,  10,    0,  -11 }   // '#'
 ,{    22,   9,  16,  10,    1,  -13 }   // '$'
 ,{    40,  16,  13,  16,    1,  -12 }   // '%'
 ,{    66,  11,  13,  12,    1,  -12 }   // '&'
 ,{    84,   2,   4,   4,    1,  -12 }   // '''
 ,{    85,   4,  17,   6,    1,  -12 }   // '('
 ,{    94,   4,  17,   6,    1,  -12 }   // ')'
 ,{   103,   5,   5,   7,    1,  -12 }   // '*'
 ,{   107,   6,   8,  11,    3,   -7 }   // '+'
 ,{   113,   2,   4,   5,    2,    0 }   // ','
 ,{   114,   4,   1,   6,    1,   -4 }   // '-'
 ,{   115,   2,   1,   5,    1,    0 }   // '.'
 ,{   116,   5,  13,   5,    0,  -12 }   // '/'
 ,{   125,   8,  13,  10,    1,  -12 }   // '0'
 ,{   138,   4,  13,  10,    3,  -12 }   // '1'
 ,{   145,   9,  13,  10,    1,  -12 }   // '2'
 ,{   160,   8,  13,  10,    1,  -12 }   // '3'
 ,{   173,   7,  13,  10,    2,  -12 }   // '4'
 ,{   185,   9,  13,  10,    1,  -12 }   // '5'
 ,{   200,   9,  13,  10,    1,  -12 }   // '6'
 ,{   215,   8,  13,  10,    0,  -12 }   // '7'
 ,{   228,   9,  13,  10,    1,  -12 }   // '8'
 ,{   243,   8,  13,  10,    1,  -12 }   // '9'
 ,{   256,   2,  10,   5,    1,   -9 }   // ':'
 ,{   259,   3,  12,   5,    1,   -8 }   // ';'
 ,{   264,   9,   9,  11,    1,   -8 }   // '<'
 ,{   275,   9,   4,  11,    1,   -5 }   // '='
 ,{   280,   9,   9,  11,    1,   -8 }   // '>'
 ,{   291,   9,  13,  10,    1,  -12 }   // '?'
 ,{   306,  17,  16,  18,    1,  -12 }   // '@'
 ,{   340,  12,  13,  12,    0,  -12 }   // 'A'
 ,{   360,  11,  13,  12,    1,  -12 }   // 'B'
 ,{   378,  11,  13,  13,    1,  -12 }   // 'C'
 ,{   396,  11,  13,  13,    1,  -12 }   // 'D'
 ,{   414,   9,  13,  11,    1,  -12 }   // 'E'
 ,{   429,   8,  13,  11,    1,  -12 }   // 'F'
 ,{   442,  12,  13,  14,    1,  -12 }   // 'G'
 ,{   462,  11,  13,  13,    1,  -12 }   // 'H'
 ,{   480,   2,  13,   5,    2,  -12 }   // 'I'
 ,{   484,   7,  13,  10,    1,  -12 }   // 'J'
 ,{   496,  11,  13,  12,    1,  -12 }   // 'K'
 ,{   514,   8,  13,  10,    1,  -12 }   // 'L'
 ,{   527,  13,  13,  15,    1,  -12 }   // 'M'
 ,{   549,  11,  13,  13,    1,  -12 }   // 'N'
 ,{   567,  13,  13,  14,    1,  -12 }   // 'O'
 ,{   589,  10,  13,  12,    1,  -12 }   // 'P'
 ,{   606,  13,  14,  14,    1,  -12 }   // 'Q'
 ,{   629,  12,  13,  13,    1,  -12 }   // 'R'
 ,{   649,  10,  13,  12,    1,  -12 }   // 'S'
 ,{   666,   9,  13,  11,    1,  -12 }   // 'T'
 ,{   681,  11,  13,  13,    1,  -12 }   // 'U'
 ,{   699,  11,  13,  12,    0,  -12 }   // 'V'
 ,{   717,  17,  13,  17,    0,  -12 }   // 'W'
 ,{   745,  12,  13,  12,    0,  -12 }   // 'X'
 ,{   765,  12,  13,  12,    0,  -12 }   // 'Y'
 ,{   785,  10,  13,  11,    1,  -12 }   // 'Z'
 ,{   802,   3,  17,   5,    1,  -12 }   // '['
 ,{   809,   5,  13,   5,    0,  -12 }   // '\'
 ,{   818,   3,  17,   5,    0,  -12 }   // ']'
 ,{   825,   7,   7,   8,    1,  -12 }   // '^'
 ,{   832,  10,   1,  10,    0,    3 }   // '_'
 ,{   834,   4,   3,   5,    0,  -12 }   // '`'
 ,{   836,   9,  10,  10,    1,   -9 }   // 'a'
 ,{   848,   9,  13,  10,    1,  -12 }   // 'b'
 ,{   863,   8,  10,   9,    1,   -9 }   // 'c'
 ,{   873,   8,  13,  10,    1,  -12 }   // 'd'
 ,{   886,   8,  10,  10,    1,   -9 }   // 'e'
 ,{   896,   4,  13,   5,    1,  -12 }   // 'f'
 ,{   903,   8,  14,  10,    1,   -9 }   // 'g'
 ,{   917,   8,  13,  10,    1,  -12 }   // 'h'
 ,{   930,   2,  13,   4,    1,  -12 }   // 'i'
 ,{   934,   4,  17,   4,    0,  -12 }   // 'j'
 ,{   943,   9,  13,   9,    1,  -12 }   // 'k'
 ,{   958,   2,  13,   4,    1,  -12 }   // 'l'
 ,{   962,  13,  10,  15,    1,   -9 }   // 'm'
 ,{   979,   8,  10,  10,    1,   -9 }   // 'n'
 ,{   989,   8,  10,  10,    1,   -9 }   // 'o'
 ,{   999,   9,  13,  10,    1,   -9 }   // 'p'
 ,{  1014,   8,  13,  10,    1,   -9 }   // 'q'
 ,{  1027,   5,  10,   6,    1,   -9 }   // 'r'
 ,{  1034,   8,  10,   9,    1,   -9 }   // 's'
 ,{  1044,   4,  12,   5,    1,  -11 }   // 't'
 ,{  1050,   8,  10,  10,    1,   -9 }   // 'u'
 ,{  1060,   9,  10,   9,    0,   -9 }   // 'v'
 ,{  1072,  13,  10,  13,    0,   -9 }   // 'w'
 ,{  1089,   8,  10,   9,    0,   -9 }   // 'x'
 ,{  1099,   9,  14,   9,    0,   -9 }   // 'y'
 ,{  1115,   7,  10,   9,    1,   -9 }   // 'z'
 ,{  1124,   4,  17,   6,    1,  -12 }   // '{'
 ,{  1133,   2,  17,   4,    2,  -12 }   // '|'
 ,{  1138,   4,  17,   6,    1,  -12 }   // '}'
 ,{  1147,   7,   3,   9,    1,   -7 }   // '~'
 ,{  1150,  12,  13,  12,    0,  -12 }   // 'Ä'
 ,{  1170,  13,  13,  14,    1,  -12 }   // 'Ö'
 ,{  1192,  11,  13,  13,    1,  -12 }   // 'Ü'
 ,{  1211,   9,  13,  10,    1,  -12 }   // 'ä'
 ,{  1227,   8,  13,  10,    1,  -12 }   // 'ö'
 ,{  1240,   8,  13,  10,    1,  -12 }   // 'ü'
 ,{  1253,   8,  13,  10,    1,  -12 }   // 'ß'
 ,{  1266,   7,   6,   9,    1,  -12 }   // '°'
};

const GFXfont TFTLabelFont PROGMEM = {
  (uint8_t  *)TFTLabelFontBitmaps,
  (GFXglyph *)TFTLabelFontGlyphs,
  0x20, 0x86, 22 };

// Approx. 1822 bytes
